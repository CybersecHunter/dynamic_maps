...
var _launchPopupSkipped = false;
var inactivityTimer = null;
var inactivityTimeout = 15000; // 20 seconds
var inactivityPopupVisible = false;
...
var Controller = function () {
  ...
  document.addEventListener("visibilitychange", function () {
    if (document.hidden) {
      ...
      if (_hasAudio == "true" || _navbarUiElementsOpen == true)
        _playbackView.pauseMedia();
      if (_hasVideo == "true" || _navbarUiElementsOpen == true)
      ...
      if (_hasVideo == "true" && _navbarUiElementsOpen == false)
        checkVideoPlayStatus("close");
    }
  });
...
Controller.prototype.checkPreLoaderHide = function () {
  ...
  if (this.preLoadImgFlg && this.jsonLoadFlg) {
    ...
    $("#f_pageCounter")
      ...
      .attr(
        "aria-label",
        "page " + (this.pageCnt + 1) + " of " + (this.totalPages + 1),
      );
    ...
    startDirectlyFromJsonPage0();
  }
};

var coursePageStarted = false;

function startDirectlyFromJsonPage0() {
  if (coursePageStarted) return;
  coursePageStarted = true;

  $("#f_start_course").stop(true, true).hide();
...
function pageLoad(e) {
  ...
  $("#skipHelpBtn").off("click").on("click", skipHelpVideo);
}
  ...
  if (
    ...
    $("#f_start_course").animate(
      ...
      function () {
        ...
        // }
        _controller.pageCnt = 0;
        if (_launchPopupSkipped) {
          console.log("Launch popup skipped → opening second page");

          _launchPopupSkipped = false;
          _controller.pageCnt = 1;
        } else {
          console.log("Normal course start → opening page_0");
...
function showLaunchIntroPopup() {
  $("#launchIntroPopup").show();
...
function closeLaunchIntroPopup() {
  var popup = document.getElementById("launchIntroPopup");
  if (!popup) {
    return;
  }
  ...
  var audio = document.getElementById("launchPopupAudio");
  if (audio) {
    audio.pause();
    audio.currentTime = 0;
  }
...
function skipLaunchIntroPopup() {
  hideLaunchIntroPopup();

  console.log("Skipping page_0 -> opening second page directly");

  // Skip page_0 completely.
  // Do NOT start the dummy-media/preloader sequence here.
  _launchPopupSkipped = false;
...
Controller.prototype.fnClick = function (e) {
  ...
  switch (id) {
    case "f_gadgetBtn":
      ...
      if (e.type == "click") {
        ...
        if (gadgetToggle) {
          ...
          //closeMenuPopup();
          gadgetBtn
            .removeClass("f_gadgetnavBtn")
            .addClass("f_gadgetClosenavBtn");
          gadgetToggle = true;
        }
      }
      break;
          ...
          $("#f_menuContainer")
            ...
            .animate(
              ...
              function () {
                //setMenuTabIndex();
                //hideGadget();
                $(".f_menuLevel_2").eq(_controller.pageCnt).focus();
                checkResizePopup();
              },
            );
          removeNavigationAllTabIndex();
    ...
    case "f_glossaryBtn":
      ...
      if (e.type == "click") {
        ...
        setTimeout(function () {
          ...
          $("#f_glossaryPopup")
            .find("#f_glossaryContainer")
            .find("#f_glossaryHeader")
            .find("#f_glossaryTitle")
            .focus();
          let glossaryContant = $("#f_glossaryContainer")
            .find("table")
            .find("tbody");
    ...
    case "f_resourceBtn":
      ...
      if (e.type == "click") {
        ...
        setTimeout(function () {
          ...
          $("#f_resourcePopup")
            .find("#f_resourceContainer")
            .find("#f_resourceHeader")
            .find("#f_resourceTitle")
            .focus();
          let resourceContant = $("#f_resourceContainer").find(
            "#f_resourceContent",
          );
    ...
    case "f_reviewBtn":
      if (e.type == "click") {
        window.open(
          "https://sites.ey.com/sites/CollaborativeReviewTool/Shared%20Documents/crt/index.aspx#/?projectName=Dark_theme_template_Advanced&projectCode=Dark_theme_template_Advanced&deliveryPhase=Alpha&serviceLine=Americas",
          "reviewWindow",
          "menubar=1,resizable=1,width=550,height=480",
        );
      }
      break;
    case "f_refrenceBtn":
      ...
      if (e.type == "click") {
        ...
        setTimeout(function () {
          ...
          $("#f_refrencePopup")
            .find("#f_refrencePopup")
            .find("#f_refrenceHeader")
            .find("#f_refrenceTitle")
            .focus();
          let refrenceContant = $("#f_refrenceContainer").find(
            "#f_refrenceContent",
          );
...
Controller.prototype.updateViewNow = function () {
  ...
  this.backDisable();
  pageCounter.html(
    "<span class='pageNumbr'>" +
      this.pageCnt +
      1 +
      "</span><span class='centerLine'></span>" +
      (this.totalPages + 1),
  );
...
function pagePreLoad() {
  ...
  setTimeout(function () {
    ...
    if (!isDevice) {
      $(".page_heading").parent().parent().attr("tabindex", -1);
    }
...
function pageVisited() {
  ...
  if (_strictNavigation) {
    _menuView.enableMenuButtonNavigation(
      _controller.pageCnt,
...
function showHelpVideo() {
  ...
  $("#helpPlayBtn").removeClass("helpPlay").addClass("helpPause");
  $("#skipHelpBtn").off("click").on("click", skipHelpVideo);
  $("#helpPlayBtn").off("click").on("click", togglePlayHelpVideo);
  $("#helpReplayBtn").off("click").on("click", replayHelpVideo);
  $("#helpProgress").off("mousedown").on("mousedown", updateHelpVideoTime);
  $("#helpProgress").off("touchstart").on("touchstart", updateHelpVideoTime);
...
function helpMediaEnded() {
  _helpVideoMedia.pause();
  $("#helpPlayBtn").removeClass("helpPause").addClass("helpPlay");
  $("#helpVideoPlayer").removeClass("videoPlayer-fade-out");
}

function updateHelpVideoTime(e) {
  var _x = 0;
  var parentX = parseInt($("#helpProgress").parent().offset().left);
  ...
  if (e.originalEvent.touches) {
    ...
    $(document).off("touchmove").on("touchmove", progressBarMouseMove);
    $(document).off("touchend").on("touchend", progressBarMouseUp);
    _x = Math.round(touchPageX - currentX);
  } else {
    helpUpdateProgressbar(e.pageX);
    _x = Math.round(e.pageX - currentX);
    $(document).off("mousemove").on("mousemove", progressBarMouseMove);
    $(document).off("mouseup").on("mouseup", progressBarMouseUp);
  }
  ...
  if (_helpVideoMedia.currentTime >= _helpVideoMedia.duration) {
  } else {
    _helpVideoMedia.play();
    $("#helpPlayBtn").removeClass("helpPlay").addClass("helpPause");
  }
...
function setButtonState(btn, isFullscreen) {
  ...
  btn.setAttribute("data-tooltip", isFullscreen ? "Normal View" : "Fullscreen");
}
...
function resetInactivityTimer() {
  ...
  // Don't start another timer while popup is open
  if (inactivityPopupVisible) {
    return;
  }
...
function showInactivityPopup() {
  ...
  var $popup = $("#inactivePopup");

  console.log("Showing inactivity popup");
  console.log("inactivePopup found:", $popup.length);
  console.log("hideMain visible:", $(".hideMain").is(":visible"));

  if (!$popup.length) {
    console.error("#inactivePopup does not exist in the DOM");
    return;
  }

  inactivityPopupVisible = true;

  $popup.stop(true, true).css("display", "flex");
}

function hideInactivityPopup() {
  inactivityPopupVisible = false;

  $("#inactivePopup").stop(true, true).hide();
...
function initInactivityDetection() {
  ...
  // Remove only our inactivity handlers
  $(document).off(".inactivity");

  // User activity
  $(document).on(
    "mousedown.inactivity " +
      "click.inactivity " +
      "keydown.inactivity " +
      "touchstart.inactivity " +
      "wheel.inactivity " +
      "mousemove.inactivity",
    function () {
      if (!inactivityPopupVisible) {
        resetInactivityTimer();
      }
  ...
  // Continue button
  $("#inactivePopupContinue")
    .off(".inactivity")
    .on("click.inactivity", function (e) {
      e.preventDefault();
      e.stopPropagation();
